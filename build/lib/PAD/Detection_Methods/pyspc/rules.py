...
#                          self.RULES_6_TRENDING, RULES_14_UP_DOWN]
    ...
    def test_violating_runs(self, data, center, ucl):
        for i in range(1, len(data)):
            if (data[i - 1] - center) * (data[i] - center) < 0:
                return False
        return True

    def test_beyond_limits(self, value,  ucl):
        return value > ucl 

    def RULE_1_BEYOND_3SIGMA(self, ax, values, center,  ucl):
        points = []
        if isinstance(ucl, list):
            for i, value in enumerate(values):
                if self.test_beyond_limits(value, ucl[i]):
            ...
            for i in range(len(values)):
                for j, value in enumerate(values[i]):
                    if self.test_beyond_limits(value, ucl):
                        ax.plot([j], value, 'ro', markersize=5)
                        points.append(j)
        else:
            for i in range(len(values)):
                if self.test_beyond_limits(values[i], ucl):
                    ax.plot([i], values[i], 'ro', markersize=5)
                    points.append(i)

        return points

    #def RULE_7_ON_ONE_SIDE(self, ax, values, center, ucl):
        #points = []
        # ewma, p Charts
        ...
        #    if i <= (num - 1):
        #        continue
        #    if self.test_violating_runs(values[i - num + 1:i + 1], center, ucl):
        #        ax.plot([i], values[i], 'yo', markersize=5)
        #        points.append(i)

       # return points

    def plot_violation_points(self, ax, values, center, ucl):
        violating_points = []
        for func in self.rules:
            violating_points += func(ax, values, center, ucl)
        b=[]
        c=list(set(violating_points))
        for i in range(len(c)):
            a=c[i]+1
            b.append(a)
 #       print("all outbreak:",b)
 #       print("the recent outbreak:",max(b))
        return list(set(violating_points)),b
